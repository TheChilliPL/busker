...
import {GuildChannel, GuildFeatures, Snowflake, TextBasedChannel, VoiceChannel} from "discord.js";
...
export async function getStatusChannel(argument: Queue | Discord.Guild): Promise<Discord.TextBasedChannel | null> {
    ...
    if(
        ...
        && guild.features.includes("TEXT_IN_VOICE_ENABLED" as GuildFeatures)
        && voiceChannel.permissionsFor(client.user)?.has("SEND_MESSAGES")
    )
        return voiceChannel;
    ...
    if(metadataChannel && !metadataChannel.deleted && metadataChannel.permissionsFor(client.user)?.has("SEND_MESSAGES"))
        return metadataChannel;

    // Fallback to the first accessible channel in the guild.
    let channels = await guild.channels.fetch();
    let channel = (channels.filter(c => c.isText()) as Discord.Collection<Snowflake, Discord.TextChannel>)
        .sort((a, b) =>
